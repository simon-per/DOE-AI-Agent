...
**4. Always review execution scripts**
After creating or modifying any script in `execution/`, you MUST invoke the reviewer sub-agent using `directives/review_execution_script.md` before considering the task complete. Use the Task tool with subagent_type="general-purpose" to get fresh-eyes feedback on correctness, efficiency, code quality, configuration, and integration. Implement CRITICAL and HIGH priority fixes immediately, then update the governing directive with learnings.
...
**Endpoints:**
- `https://<your-modal-app>--claude-orchestrator-list-webhooks.modal.run` - List webhooks
- `https://<your-modal-app>--claude-orchestrator-directive.modal.run?slug={slug}` - Execute directive
- `https://<your-modal-app>--claude-orchestrator-test-email.modal.run` - Test email
