...
- **Selection:** Sheet rows with `Score >= --min-score`, `CL_Generated=Yes`, `Status` NOT in `FROZEN_STATUSES` (Applied / APPLYING / Follow-Up_Sent / Interviewing / Rejected / Offer / No_Response / Expired / Duplicate / PAUSED / FAILED), and an existing `.tmp/applications/{Job_ID}_*/Cover_Letter_Simon_Oberpertinger_Mair.docx`. Frozen statuses are historical submissions whose letter date must NOT be rewritten.
- **Idempotency (2026-05-10):** Per-job loop skips folders whose DOCX is already stamped with `--letter-date` AND has a `.drive_pushed_{YYYY-MM-DD}` marker. Marker is dropped on successful Drive push and stale markers are swept; both gates together prevent silent Drive-divergence when a transient upload failure is logged but not raised.
- **Drive push scope:** After re-stamping, only the cover letter pair + `letter_meta.json` are pushed back to Drive (via `upload_application_folder(..., only_files=...)`). The unchanged CV / ATS pair is left alone, cutting per-job Drive time roughly in half.
...
- **Daily restamp must be bounded to the active set (2026-05-10):** Modal `pipeline_daily_maintenance` timed out (3 600 s wall) on 2026-05-09 and 2026-05-10 because `update_cover_letter_dates` was re-stamping every score≥6 letter — including all historical Applied / Interviewing / Rejected rows — and pushing all 7 files in each folder to Drive even though only the cover-letter pair changed. Three fixes shipped together: (a) `FROZEN_STATUSES` filter at sheet load (frozen rows are immutable submissions), (b) per-job idempotency probe that skips folders already stamped today and confirmed pushed via marker file, (c) Drive `only_files=` allowlist scoped to the cover letter pair + `letter_meta.json`.
- **Round-2 hardening after the restamp fix (2026-05-10):** A fresh-eyes audit of the Round-1 changes plus the surrounding pipeline turned up six gaps closed in a follow-up pass:
  - **Dot-file upload filter (`drive_upload.py:upload_application_folder`)** — `iterdir()` was picking up the new `.drive_pushed_*` markers (and any `.DS_Store` / `.part` temps) and would have uploaded them to Drive on a `--reset-checkpoint` re-run of generate_cover_letter / generate_cv. Default mode now skips dot-prefixed files; the `only_files` allowlist path is unaffected.
  - **Drive HTTP socket timeout** — every `build("drive", "v3", ...)` call site routes through `_build_drive_service(creds)` which wires `httplib2.Http(timeout=60)` via `AuthorizedHttp`. Caps fully-stalled sockets (the realistic failure mode that pinned `pipeline_full` to its outer wall on 2026-05-09); does NOT cap a slow-but-progressing upload — see the constant's comment.
  - **Marker write isolated from Drive try/except** — a marker `.touch()` failure used to be conflated with Drive failure inside one `try`; both silently downgraded the post-mortem signal AND broke tomorrow's skip-if-today guard. Marker write now runs in its own `try` after `upload_succeeded=True`, and an `OSError` logs at `log.error` (loud enough to surface in Modal alert tails) without rolling back the successful render+upload.
  - **Outer-wall timeouts cover per-stage budget sums** — `pipeline_full` was 14 400 s vs a per-stage sum of 18 600 s (guaranteed overflow on any slow stage). Bumped to 19 800 s. `pipeline_daily_maintenance` was 7 200 s vs sum 7 200 s (zero margin). Bumped to 8 400 s. Margins are 1 200 s each; comments at the call sites show the math.
  - **Error-rate denominator now coverage-aware** — `attempted = updated + errored + skipped_already_today` so a quiet day with 1 failure but 7 correctly skipped folders doesn't false-alarm at 33 %. `missing_folder` / `missing_docx` stay separate (they have their own `_send_coverage_alert`).
  - **Loud warning on missing Status column** — if Sheet headers ever drift such that the `Status` column disappears, `has_status_col=False` silently disabled the frozen-status filter. Now emits a `log.warning` at sheet-load time naming the regression risk.
