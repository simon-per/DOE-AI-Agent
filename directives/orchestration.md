...
**Applied row age-out:** `execution/prune_stale_jobs.py` also deletes rows stuck at `Status=Applied` when `Date_Applied` is older than `--applied-days` (default 45 calendar days) and no `Response_Date`, `Interview_Date`, or `Response_Received=Yes` marker exists. This keeps ghosted February/March-style applications from living forever while preserving any row with outcome evidence. Deleted rows feed the existing Drive archive and orphan-folder cleanup path. The tool deletes these rows directly rather than marking them `Rejected`, because ghosted rows should not be counted as actual responses in the weekly metrics.
...
**Preflight (`execution/preflight.py`)** — on-demand Modal/local connectivity
check for deploy validation and manual smoke tests. It is intentionally not a
scheduled `pipeline_full` gate because it mutates Sheets/Drive, sends a Gmail
test email, and pings LLM models; transient failures there should not block a
normal scrape/evaluate/generate run. 11 checks:
...
If any check fails during an explicit preflight run, an alert email is sent to
`simonobemair@gmail.com` and Modal marks that preflight run as failed.
...
is then re-raised so Modal also flags the run. Each stage also streams to
`.tmp/stage_logs/{timestamp}_{stage}.log` on the Modal Volume; timeout/non-zero
paths commit the Volume before raising so post-mortems do not depend only on
Modal's ephemeral log stream.
...
- **Round 3 lifecycle + OAuth (2026-05-07):** every cloud cron now runs `_check_oauth_health` at startup — actively reads the Sheet header, hashes `token.json` to detect rotation via `oauth_sentinel.json` on Volume, sends `[OAUTH WARN]` once on day-5 and once on day-6, hard-fails with `[OAUTH FAIL]` from day-7. Modal and local fallback wrappers pass `prune_stale_jobs.py --days 14` so both Sheet-mutating paths use the same relevance window. Drive folders for pruned rows now move to `DOE Applications/_Archive/` (stamped with `appProperties.archived_at`); folders older than 14 days are permanently deleted on the next prune run. `prune_stale_jobs.py --archive-drive` is the cloud-only opt-in (local runs continue without Drive archiving). Schema-drift errors in `update_cover_letter_dates.py` now print all available headers + a 30-second remediation hint instead of a one-line opaque RuntimeError.
